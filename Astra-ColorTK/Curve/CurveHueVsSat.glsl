...
float graphEvalWrap(float graph_x, CurveHueVsSat gp)
{
...
struct CurveHueVsSat {
	sampler2D sampler;

	//@ label: "mix_CurveHueVsSat", editor: range, min: 0, max: 1, range_min: 0, range_max: 1, range_default: 1
	float mix;
...
vec4 texture( CurveHueVsSat s, vec2 tex_coords )
{
	//input
	...
	//graph_x
	float graph_x = rgb2hsv(color.rgb).x;


	//graph
	float eval = graphEvalWrap(graph_x + epsilon, s);


	//adjustment
    vec4 col = vec4( adjustSAT( color.rgb, eval, yCOE ), color.a );
