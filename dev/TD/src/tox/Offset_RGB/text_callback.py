...
def onTextEditEnd(comp, value, prevValue):
	...
	if comp.name == 'y':
		...
		try:
			...
			parent.dial.RGB.val[chIndex] = new_val + parent.dial.Default['Offset']
			parent.dial.Value.val = max(parent.dial.RGB.val)
			parent.dial.UpdatePos()
			comp.par.text.expr = "parent.dial.UIRGB.val["+str(chIndex)+"]"
		except:
			comp.par.text.expr = "parent.dial.UIRGB.val["+str(chIndex)+"]"

		parent.dial.op('eval1').cook(force=True) #another dependency hack
	return
