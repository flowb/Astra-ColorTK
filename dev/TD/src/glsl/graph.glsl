...
//reordered fields on mix() for better inline readability
float xim( float x, float a, float b )
{
	return mix(b,a,x);
}

float graphEvalWrap(float graph_x, graphPoints gp)
{
	...
	vec2 pR,pL;
	float c0,c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,c17,cL,eval;
...
float graphEval(float graph_x, graphPoints gp)
{
	vec2 p1 = vec2(gp.p1_X, gp.p1_Y);
	vec2 p2 = vec2(gp.p2_X, gp.p2_Y);
	vec2 p3 = vec2(gp.p3_X, gp.p3_Y);
